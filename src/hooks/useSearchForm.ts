...
export const useSearchForm = (initialValues?: SearchFormInitialValues) => {
    ...
    // FORM STATE
    // Inisialisasi awal (akan di-overwrite oleh useEffect di bawah saat data matang)
    ...
    // --- [FIX UTAMA] SINKRONISASI DATA ---
    // Efek ini akan jalan setiap kali 'initialValues' berubah (misal: saat data API di parent selesai loading)
    useEffect(() => {
        if (initialValues) {
            // Cek satu per satu untuk memastikan data ter-update
    ...
    useEffect(() => {
        const fetchAirports = async () => {
            try {
                ...
                // Default logic JIKA initialValues benar-benar kosong (Case: Landing Page fresh load)
                // Kita cek props initialValues undefined/null agar tidak menimpa data yang sedang loading
        ...
        fetchAirports();
    }, []); // Dependency kosong agar hanya fetch sekali saat mount
